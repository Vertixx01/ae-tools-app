<script lang="ts">
  ...
  let { installs, loading, busy, onSetPerformanceMode, onClearInstall, onOpenPath }: Props = $props();
</script>

<div class="panel rounded-[28px] p-5 md:p-6">
  <div class="mb-5 flex items-center justify-between gap-4">
    <div>
      <p class="mono text-[11px] uppercase tracking-[0.24em] text-[color:var(--muted)]">After Effects installs</p>
  ...
  <div class="grid gap-4">
    {#if loading}
      <div class="rounded-[24px] border border-white/8 bg-white/4 p-5 text-[color:var(--muted)]">Scanning Adobe installs and version folders...</div>
    {:else if installs.length}
      {#each installs as install}
        <InstallCard {install} {busy} {onSetPerformanceMode} {onClearInstall} {onOpenPath} />
      {/each}
    {:else}
      <div class="rounded-[24px] border border-white/8 bg-white/4 p-5 text-[color:var(--muted)]">No installs detected yet.</div>
    {/if}
  </div>
</div>
